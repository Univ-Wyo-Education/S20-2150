...
module.exports = {
	...
	, msg: function ( wire, val ) {
		switch ( wire ) {
		case "Clr": if ( val === 1 ) { my["_data_"] = 0; }									TurnOn( "input_Clr" );   Display( my["_data_"]); break;
		case "Ld":  if ( val === 1 ) { PullBus(true); my["_data_"] = my["_InputBuffer_"]; }				TurnOn( "input_Ld"  );   Display( my["_data_"]); my["_Ld_"] = 1; break;
		case "Inc": if ( val === 1 ) { my["_data_"] = my["_data_"] + 1; }	    			TurnOn( "input_Inc" );   Display( my["_data_"]); break;
		case "Out": if ( val === 1 ) { my["_OutputBuffer_"] = my["_data_"]; PushBus(); }   TurnOn( "input_Out" );   Display( my["_data_"]); break;
